...
select_localizacoes = """SELECT L.id, L.titulo, L.conteudo, L.aberto, L.mascara, L.toalha, L.bebedouro, L.vestiario
FROM LOCALIZACOES AS L;"""

select_localizacoes_abertas = """SELECT L.id, L.titulo, L.conteudo, L.aberto, L.mascara, L.toalha, L.bebedouro, L.vestiario
FROM LOCALIZACOES AS L WHERE L.aberto = 1"""

count_agendamentos = """SELECT count(A.id_localizacao) FROM AGENDAMENTOS as A WHERE A.id_localizacao = %s AND A.horario in (%s);
"""

select_agendamentos = """SELECT A.dias_semana, A.horario FROM AGENDAMENTOS as A WHERE A.id_localizacao = %s;
"""
